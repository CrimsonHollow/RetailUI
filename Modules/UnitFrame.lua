...
local customtexture = addonpath .. "\\Textures\\Status\\ToxiUI-clean"
local h = 5
local w = 225
...
local function ReplaceBlizzardPlayerFrame(frame)
    ...
    -- healthBar:SetPoint("TOPLEFT", 74, -18)
    -- healthBar:SetSize(121, 27)
	healthBar:SetPoint("TOPLEFT", 72, -15)
    healthBar:SetSize(123, 31)
	...
	-- statusBarTexture:SetTexture(customtexture)

    local manaBar = _G[playerFrame:GetName() .. 'ManaBar']
    manaBar:SetFrameLevel(playerFrame:GetFrameLevel() + 2)
	
    manaBar:ClearAllPoints()
    manaBar:SetPoint("CENTER", UIParent, "CENTER", 0, -175)
    manaBar:SetSize(206, 9)
	-- manaBar:Hide()
...
local function ReplaceBlizzardRuneFrame()
    for index = 1, 6 do
        ...
        if index > 1 then
            ...
            button:SetPoint('CENTER', UIParent, -55, -170)
        end
...
local function ReplaceBlizzardTargetFrame(frame, target, isBoss)
    ...
    healthBar:ClearAllPoints()
    healthBar:SetPoint("TOPLEFT", 5, -14)
    healthBar:SetSize(118, 20)
    ...
    manaBar:ClearAllPoints()
    manaBar:SetPoint("TOPLEFT", 5, -38)
    manaBar:SetSize(132, 5)
    ...
    healthText:SetDrawLayer("OVERLAY")
	healthText:SetFont(customfont , 10, "OUTLINE") 
