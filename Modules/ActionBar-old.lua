...
local function CreateActionFrameBar(barID, buttonCount, buttonSize, gap, vertical, frameName)
	...
	frameBar:SetFrameStrata("LOW")  -- Or "MEDIUM" if needed; avoid "BACKGROUND" for main bars
	frameBar:EnableMouse(true)  -- Ensure clickable
    ...
    frameBar.ID = barID
	if not frameBar.ID then
    print("Error: frameBar.ID not set for bar " .. (frameName or "unknown"))
end
...
-- ActionBar.lua (cleaned + mouseover fade working for 3.3.5)


-- Smooth fade helper (3.3.5-safe)
...
local function SetupMouseoverActionBar(bar)
  ...
  local function Hide() FadeFrame(bar, 0, 0.25) end
local function IsDraggingSpell()
    return GetCursorInfo()  -- Returns info if dragging a spell/action
end

local oldLeave = bar:GetScript("OnLeave")
bar:SetScript("OnLeave", function(self, ...)
    if oldLeave then oldLeave(self, ...) end
    if not IsDraggingSpell() then
        Hide()
    end  -- Only hide if not dragging
end)
  -- hook bar itself
  ...
  -- hook all actual Blizzard buttons belonging to this bar
  local prefixes = {
...
local function ReplaceBlizzardActionBarFrame(frameBar)
    ...
    if not blizzActionBars[frameBar.ID] then
        print("Error: Invalid frameBar.ID " .. (frameBar.ID or "nil") .. " for bar " .. frameBar:GetName())
        return
    end
    ...
    for index = 1, frameBar.buttonCount do
        ...
        if not button then
            print("Error: Button " .. blizzActionBars[frameBar.ID] .. index .. " not found")
            return
        end
        ...
        button:SetHitRectInsets(-5, -5, -5, -5) -- Added for drag reliability
...
if RUI.DB.profile.widgets.useCompactRightBar then
    local rightBar = Module.actionBars[4]  -- MultiBarRight
    MultiBarRightButton1:ClearAllPoints()
    MultiBarRightButton1:SetFrameStrata("LOW")  -- Changed from BACKGROUND
    MultiBarRightButton1:SetPoint("CENTER", UIParent, "CENTER", -90, -220)
    ...
    for _, index in ipairs({6, 7, 8, 9, 10, 11, 12}) do
        local button = _G["MultiBarRightButton" .. index]
        if button then
            button:SetAlpha(0)
        end
    end
    ...
    MultiBarRightButton1:ClearAllPoints()
    MultiBarRightButton1:SetPoint("TOPLEFT", MultiBarLeftButton1, "TOPRIGHT", 3, 0)
    MultiBarRightButton1:SetFrameStrata("LOW")
    MultiBarRightButton1:EnableMouse(true)
...
function Module:LoadDefaultSettings()
	...
	RUI.DB.profile.widgets.useCompactRightBar = true -- default: ON
