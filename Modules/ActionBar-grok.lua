...
local function CreateActionFrameBar(barID, buttonCount, buttonSize, gap, vertical, frameName)
    ...
    frameBar:SetFrameStrata("LOW")  -- Ensure clickable strata
    frameBar:EnableMouse(true)      -- Ensure mouse input
    ...
    if barID == MAIN_ACTION_BAR_ID then
        ...
        nineSliceFrame:SetParent(frameBar)  -- Fixed: Parent to frameBar
...
local function SetupMouseoverActionBar(bar)
    ...
    local function Hide() 
        if not GetCursorInfo() then -- Prevent hide during drag
...
local function ReplaceBlizzardActionBarFrame(frameBar)
    ...
    for index = 1, frameBar.buttonCount do
        ...
        button:SetHitRectInsets(-5, -5, -5, -5) -- For drag reliability

        local normalTexture = button:GetNormalTexture()
        if not normalTexture then
            normalTexture = button:CreateTexture(nil, "BACKGROUND")
...
local function RemoveBlizzardFrames()
    local blizzFrames = {
        ...
        MainMenuXPBarTextureMid,  -- Added for XP bar
        ExhaustionTick,           -- Added for exhaustion marker
        MainMenuBarOverlayFrame,  -- Added for overlay
